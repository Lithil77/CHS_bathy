...
function LayerSwitcher() {
    ...
    const { olMap, ConfigWMSLayerToMap, updateLayerSwitcherFlag, updateMapLayerConfigs,
        mapLayerConfigs, depthFileterSelectedlayerFlag, depthFileterSelectedlayer,
        updateDepthFileterSelectedlayerFlag, clearVectorSource } = useContext(OLMapContext);

    const { backgroundColor, textColor, borderColor, typoColor, fontFamily, cardbodyColor } = useColor();
    const { toggleComponent, isLayerSwitcherButtonActive } = useUtility();
        ...
        try {
            ...
            if (isBuilder) {
                ...
                if (data) {
                    ...
                    const filteredLayers = newLayerList.filter(layer => layer !== "S102 Bathy Raster" && layer !== "BSH S102 Bathy Raster");
                    setSelectedItems(filteredLayers);
                    updateMapLayerConfigs(data);
                ...
                if (mapLayers.length > 0) {
                    ...
                    updateMapLayerConfigs(mapLayers);
                    setLayersList(layers);
                    const filteredLayers = layers.filter(layer => layer !== "S102 Bathy Raster" && layer !== "BSH S102 Bathy Raster");
                    setSelectedItems(filteredLayers);
                    updateLayerVisibility(filteredLayers);
                }
            }
    ...
    useEffect(() => {
        ...
        if (depthFileterSelectedlayerFlag === true && depthFileterSelectedlayer !== null) {

            const allLayers = layersList.filter(layer => layer !== "S102 Bathy Raster" && layer !== "BSH S102 Bathy Raster");

            const layerConfig = mapLayerConfigs.find(lyr => lyr.layer === depthFileterSelectedlayer);

            if (layerConfig) {
                const newLayer = getLayerByName(depthFileterSelectedlayer);
                if (!newLayer) {
            ...
            allLayers.forEach(layer => {
                if (layer !== depthFileterSelectedlayer) {
            ...
            const checkedLayer = layersList.filter(layer => layer == depthFileterSelectedlayer);
            setSelectedItems(checkedLayer);
            updateDepthFileterSelectedlayerFlag(false);
        }
    }, [depthFileterSelectedlayer, depthFileterSelectedlayerFlag]);
    ...
    useEffect(() => {
        const selectableLayers = layersList.filter(layer => layer !== "S102 Bathy Raster" && layer !== "BSH S102 Bathy Raster");
    ...
    const handleSelectAllChange = (event) => {
        ...
        if (isChecked) {
            ...
            allLayers.forEach(layer => {
                const layerConfig = mapLayerConfigs.find(lyr => lyr.layer === layer);
                if (layerConfig) {
                    const newLayer = getLayerByName(layer);
                    if (!newLayer) {
                        ConfigWMSLayerToMap(olMap, layerConfig.url, layerConfig.workspace, layerConfig.layer);
    ...
    const handleCheckboxChange = (event) => {
        ...
        setSelectedItems((prevSelected) => {
            if (checked) {
                ...
                if (!layerExists) {
                    const layerConfig = mapLayerConfigs.find(lyr => lyr.layer === value);
                    if (layerConfig) {
                        ConfigWMSLayerToMap(olMap, layerConfig.url, layerConfig.workspace, layerConfig.layer);
    ...
    const handleDragOver = (index, e) => {
        ...
        updatedLayers.forEach((layerName) => {
            const layerConfig = mapLayerConfigs.find((lyr) => lyr.layer === layerName);
            if (layerConfig) {
