...
function DepthFilter() {
    ...
    const { olMap, updateDepthFileterSelectedlayer, mapLayerConfigs,
    ...
    const { showDepthFilterPopUpContainer, toggleComponent } = useUtility();
    ...
    return (
        <div id="search-control" className='mt-2 position-absolute top-0 start-0' style={{ width: 'auto' }}>
            <Row>
                    ...
                    <div>
                        <OverlayTrigger trigger="click" key="bottom" placement="bottom" className="w-25 position-absolute" show={showDepthFilterPopUpContainer} rootClose={true} overlay={
                            ...
                            <StyledButton title='Depth Filter' id='attributeQueryToggleBtn'
                                className={`p-1 mb-1`}
                                onClick={() => {
                                    toggleComponent("DepthFilter", olMap);
                                    handleListClear();
                                    clearVectorSource();
                                    if (mapLayerConfigs) {
                                        const mismatchItems = mapLayerConfigs.filter(item =>
