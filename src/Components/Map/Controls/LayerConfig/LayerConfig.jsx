...
import { Col, Row, Card, Table, Button, Form, ListGroup } from 'react-bootstrap';
...
import MapPreview from '../../../utils/MapPreview/MapPreview';
import { OlMapPreviewContext } from '../../../utils/MapPreview/MapPreviewContext';
...
function LayerConfig() {
    ...
    const { olMap, ConfigWMSLayerToMap, getAllVisibleLayers,
        geoServerWorkSpaceList, getWorkSpaces } = useContext(OLMapContext);
    ...
    const [addedLayers, setAddedLayers] = useState(() => {
        const savedLayers = localStorage.getItem('addedLayers');
        return savedLayers ? JSON.parse(savedLayers) : [];
    });

    useEffect(() => {
        localStorage.setItem('addedLayers', JSON.stringify(addedLayers));
    }, [addedLayers]);
    ...
    const addUniqueLayers = (newLayers) => {
        setAddedLayers((prevLayers) => {
            ...
            const updatedLayers = [...prevLayers, ...filteredLayers];
            localStorage.setItem('addedLayers', JSON.stringify(updatedLayers));
                ...
                <div>
                    ...
                    <Row className='mx-0 mt-2'>
                        ...
                        <Col sm={6} className='px-0'>
                            <Card>
                                <MapPreview  isLocalArea={false} isEditMode={false} />
